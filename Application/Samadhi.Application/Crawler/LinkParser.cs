...
namespace Samadhi.Application.Crawler
{
    public class LinkParser
    {
        ...
        public void ParseLinksAgility(string body, string sourceUrl)
        {
        ...
        private void FixOrTossURLs(string sourceUrl, List<string> srcList)
        {
            for(int x=0; x<srcList.Count; x++)
            {
                ...
                if (IsExternalUrl(srcList[x]))
                {
                    srcList.RemoveAt(x);
                    x--;
                    continue;
                }
                //else if (!IsAWebPage(earl))
                //{
                //    string fixedEarl = FixPath(sourceUrl, earl);
                //    _otherUrls.Add(fixedEarl);
                //}
        ...
        {
            //if ((url.Length >= 7 && url.Substring(0, 7) == "http://") || (url.Length >= 8 && url.Substring(0, 8) == "https://"))
            //{
            //    try
            //    {
            //        if (Uri.Compare(new Uri(url), CrawlerContext.URI, UriComponents.Host, UriFormat.SafeUnescaped, StringComparison.CurrentCulture) != 0)
            //            return true;
            //        else
            //            return false;
            //    }
            ...
            if ((url.Length >= 7 && url.Substring(0, 7) == "http://") || (url.Length >= 8 && url.Substring(0, 8) == "https://") || url.Length >= 2 && url.Substring(0,2)=="//")
            {
                ...
                try
                {
                    ...
                    {
                        if (tempDomain.Equals(GetDomain.GetDomainFromUrl(CrawlerContext.URI)))
                            return false;
